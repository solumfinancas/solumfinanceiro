...
export const TasksTab: React.FC = () => {
  ...
  const filteredAndSortedTasks = useMemo(() => {
    let result = tasks.filter(t => {
      // Esconde tarefas arquivadas em todas as abas
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
          ...
          <p className="text-3xl font-black tracking-tighter">{tasks.filter(t => !t.completed && !t.archived).length}</p>
        </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
          ...
          <p className="text-3xl font-black tracking-tighter text-rose-500">
            {tasks.filter(t => !t.completed && !t.archived && isPast(parseISO(t.deadline)) && !isToday(parseISO(t.deadline))).length}
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
          ...
          <p className="text-3xl font-black tracking-tighter text-emerald-500">{tasks.filter(t => t.completed && !t.archived).length}</p>
        </div>
      </div>
