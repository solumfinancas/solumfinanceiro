...
export const ClientsTab: React.FC<ClientsTabProps> = ({ 
  ...
  const activeClientsUsage = useMemo(() => {
    return clients
      ...
      .reduce((sum, c) => sum + (c.personal_imports_count || 0) + (c.business_imports_count || 0), 0);
  }, [clients]);

  const educatorUsage = useMemo(() => {
    return (profile?.personal_imports_count || 0) + (profile?.business_imports_count || 0);
  }, [profile]);
  ...
  const consolidatedLimit = useMemo(() => {
    if (ownLimitByPlan === Infinity) return clientsLimitByPlan;
    return clientsLimitByPlan + ownLimitByPlan;
  }, [clientsLimitByPlan, ownLimitByPlan]);

  const consolidatedUsage = useMemo(() => {
    if (ownLimitByPlan === Infinity) return activeClientsUsage;
    return activeClientsUsage + educatorUsage;
  }, [activeClientsUsage, educatorUsage, ownLimitByPlan]);
  ...
  const getClientLimit = (client: ClientProfile, type: 'personal' | 'business') => {
    const customLimit = type === 'personal' ? client.personal_imports_limit : client.business_imports_limit;
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="space-y-6 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
            ...
            <div className="space-y-3 bg-card border p-5 rounded-3xl shadow-sm">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-1.5 mt-2">
                        ...
                        {/* Uso de IA Individual */}
                        <div className="mt-3 pt-2.5 border-t border-border/40">
                          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[9px] font-bold text-muted-foreground">
                            <span className="text-[8px] font-black uppercase tracking-widest text-primary/80">Uso IA:</span>
                            <span className="text-slate-900 dark:text-slate-200">Pessoal {client.personal_imports_count || 0}/{getClientLimit(client, 'personal')}</span>
                            <span className="text-muted-foreground/30">|</span>
                            <span className="text-slate-900 dark:text-slate-200">Empresarial {client.business_imports_count || 0}/{getClientLimit(client, 'business')}</span>
                          </div>
                        </div>
                      </div>
                    </div>
