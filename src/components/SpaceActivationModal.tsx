...
import { User, Building2, Rocket, ArrowRight, X, Lock, ShieldCheck, AlertCircle } from 'lucide-react';
...
export const SpaceActivationModal: React.FC<SpaceActivationModalProps> = ({ 
  ...
  // 3. Não tem autorização explícita (can_activate_second_space)
  // 4. Não está em modo de gestão (educador ativando para cliente)
  const isSecondSpace = currentInitialized.length >= 1;
  const isUserRole = profile?.role === 'user';
  const isManagementMode = !!targetUserId;
  const isAuthorized = profile?.can_activate_second_space;

  const isLocked = isUserRole && isSecondSpace && !isAuthorized && !isManagementMode;
        ...
        if (targetUserId) {
          ...
          const { data, error } = await supabase.functions.invoke('admin-create-user', {
            body: {
              ...
              userId: targetUserId,
              userData: {
                initialized_spaces: [...currentSpaces, spaceType],
                business_cnpj: spaceType === 'business' ? cnpj : currentMetadata?.business_cnpj,
        ...
        >
          ...
          {isLocked ? (
            <div className="text-center py-4">
              ...
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-4 leading-relaxed mb-8">
            ...
            <>
              <div className="text-center mb-8">
                ...
                {isSecondSpace && isAuthorized && !isManagementMode ? (
                  <div className="mt-4 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center gap-3 text-emerald-600 animate-in fade-in slide-in-from-top-2 duration-500">
                    <ShieldCheck size={20} className="shrink-0" />
                    <p className="text-[10px] font-black uppercase tracking-widest text-left leading-tight">
                      Você tem autorização para ativar este segundo espaço! Deseja prosseguir?
                    </p>
                  </div>
                ) : (
